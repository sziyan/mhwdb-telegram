...
def monsters(update, context):
    ...
    weakness = []
    ailments = []
    name = " ".join(context.args).lower() #convert arguments from user message into 1 single string

    if update.message.chat.id == -1001336587845:
        if name == 'lionel':
            message = '<b>Name:</b> Lionel \n' \
                      '<b>Description:</b> Will hunt and torture any monsters to the end of the world. \n' \
                      '<b>Weakness:</b> None \n' \
                      '<b>Ailments:</b> All types'
            update.message.reply_html(message)
            return
        elif name == 'jarrod':
            message = '<b>Name:</b> Jarrod \n' \
                      '<b>Description:</b> Colonel Sanders of the fifth fleet - Odd squad. He likes to fry chicken during his free time and busy time. \n' \
                      '<b>Skills:</b> \n1) Ninjutsu Dragon Crawl (Why crawl? We dont know) \n2) KENTUCKY FRIED CHICKEN!!! (No one knows why he yells that when wounding monsters) \n' \
                      '<b>Wealkness:</b> \n1)Human size chickens walking around in gathering hall (3 stars) \n2)Saitama coming to challenge arm wrestling.... (2 stars) \n' \
                      '<b>Ailments:</b> Ailments: Will mysteriously come only at 11pm....'
            update.message.reply_html(message)
            return
...
def skills(update,context):
    ...
    if result != 'invalid':
        ...
        update.message.reply_text("Unable to find skill. Iceborne data not yet updated.")
